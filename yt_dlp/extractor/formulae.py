...
class BrightcoveLegacyIE(InfoExtractor):
    ...
    def _build_brightcove_url(cls, object_str):
        ...
        object_str = re.sub(r'(<param(?:\s+[a-zA-Z0-9_]+="[^"]*")*)>', lambda m: m.group(1) + '/>', object_str)
        ...
        if fv_el is not None:
            flashvars = dict((k, v[0]) for k, v in urllib.parse.parse_qs(fv_el.attrib['value']).items())
        else:
            flashvars = {}
    ...
    def _make_brightcove_url(cls, params):
        return update_url_query('https://c.brightcove.com/services/viewer/htmlFederated', params)
    ...
    def _extract_brightcove_urls(cls, webpage):
        ...
        if matches:
            return list(filter(None, [cls._build_brightcove_url_from_js(custom_bc) for custom_bc in matches]))
        return [
            src
    ...
    def _real_extract(self, url):
        ...
        if video_player:
            ...
            if publisher_id:
                brightcove_new_url = (
                    f'https://players.brightcove.net/{publisher_id}/default_default/index.html?videoId={video_id}'
                )
                if referer:
    ...
    def _parse_brightcove_metadata(self, json_data, video_id, headers={}):
        ...
        for text_track in json_data.get('text_tracks', []):
            ...
            if not text_track_url:
                continue
            lang = (str_or_none(text_track.get('srclang')) or str_or_none(text_track.get('label')) or 'en').lower()
...
class BrightcoveNewIE(BrightcoveNewBaseIE):
    ...
    def _extract_brightcove_urls(ie, webpage):
        ...
        for video, script_tag, account_id, player_id, embed in re.findall(
            ...
            if not script_tag and not ie._is_valid_url(bc_url, video_id, 'possible brightcove video'):
                continue
    ...
    def _real_extract(self, url):
        ...
        def extract_policy_key():
            ...
            config = self._download_json(base_url + 'config.json', video_id, fatal=False) or {}
            policy_key = try_get(config, lambda x: x['video_cloud']['policy_key'])
            if not policy_key:
                webpage = self._download_webpage(base_url + 'index.min.js', video_id)

                catalog = self._search_regex(r'catalog\(({.+?})\);', webpage, 'catalog', default=None)
                if catalog:
                    catalog = self._parse_json(js_to_json(catalog), video_id, fatal=False)
                    if catalog:
        ...
        if content_type == 'playlist':
            return self.playlist_result(
                [
                    ...
                    for vid in json_data.get('videos', [])
                    if vid.get('id')
                ],
                json_data.get('id'),
                json_data.get('name'),
        ...
        return self._parse_brightcove_metadata(json_data, video_id, headers=headers)
